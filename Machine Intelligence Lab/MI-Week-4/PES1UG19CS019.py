...
class KNN:
    ...
    def find_distance(self, x):
        ...
        dist = []
        for row in x:
            sum = np.sum(np.abs(row - self.data) ** p, axis =1, keepdims=True) ** (1/p)
            sum = np.squeeze(sum)
            dist.append(sum)
    ...
    def k_neighbours(self, x):
        ...
        """
        dist_list = self.find_distance(x)
        idx_of_neigh = np.argsort(dist_list, axis= 1)[:,:self.k_neigh]
        dist_list_sorted = np.sort(dist_list, axis =1)
        neigh_dists = dist_list_sorted[:,:self.k_neigh]
    ...
    def predict(self, x):
        ...
        target_labels = np.array([self.target[i] for i in idx_of_neigh.tolist()])
        prediction = []

        if self.weighted:
            weight_list = 1/(neigh_dists + 1e-8)
            for i in range(target_labels.shape[0]):
                unique_labels = np.unique(target_labels[i])
                unique_labels_dict = {key:0 for key in unique_labels}
                for k in range(target_labels.shape[1]):
                    unique_labels_dict[target_labels[i][k]] += weight_list[i][k]   
                keys,values = list(unique_labels_dict.keys()),list(unique_labels_dict.values())
                prediction.append(keys[np.argmax(values)])
            return prediction

        else:
            mode  = [np.unique(i)[np.unique(i, return_counts=True)[1].argmax()] for i in target_labels]
            return mode
    ...
    def evaluate(self, x, y):
        ...
        accuracy = (np.sum(prediction == y) / len(y)) * 100
        return accuracy
