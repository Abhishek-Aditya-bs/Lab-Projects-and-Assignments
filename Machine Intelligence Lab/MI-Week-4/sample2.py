...
def test_case1():
    ...
    try:
        assert model.evaluate(sample, np.array([0, 1])) == 100.0
        print("Test Case 5 for the function evaluate PASSED")
    except Exception as e:
        print("Test Case 5 for the function evaluate FAILED")

def test_case2():
    data = np.array([[0.68043616, 0.39113473, 0.1165562 , 0.70722573, 3],
       [0.67329238, 0.69782966, 0.73278321, 0.78787406, 3],
       [0.56134898, 0.25358895, 0.10497708, 0.05846073, 1],
       [0.6515744 , 0.85627836, 0.44305142, 0.53280211, 2],
       [0.47014548, 0.18108572, 0.3235044 , 0.45490616, 2],
       [0.33544621, 0.51322212, 0.98769111, 0.53091437, 4],
       [0.4577167 , 0.80579291, 0.19350921, 0.46502849, 4],
       [0.25709202, 0.06937377, 0.92718944, 0.54662592, 1],
       [0.07637632, 0.3176806 , 0.74102328, 0.32849423, 1],
       [0.2334587 , 0.67725537, 0.4323325 , 0.38766629, 3]])

    X_train = data[:, :4]
    y_train = data[:, 4]
    samples = np.array([[0.41361609, 0.45603303, 0.33195254, 0.09371524, 1],
       [0.19091752, 0.07588166, 0.03198771, 0.15245555, 1],
       [0.29624916, 0.80906772, 0.35025253, 0.78940926, 3],
       [0.96729604, 0.89730852, 0.39105022, 0.37876973, 3],
    ...
    try:
        assert model.evaluate(X_test, y_test) == 40
