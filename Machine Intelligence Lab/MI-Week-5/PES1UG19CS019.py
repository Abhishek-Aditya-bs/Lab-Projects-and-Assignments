...
class Tensor:
    ...
    def grad_matmul(self, gradients=None):
        ...
        #  4*5   =    4*6 6*5   
        #  5*6   =    5*4 4*6                 
        grad_a = gradients @ (self.history[2].arr.T)
        grad_b = self.history[1].arr.T @ (gradients)
    ...
    def backward(self, gradients=None):
        ...
        gradFuntion = self.grad_add if self.history[0] == 'add' else self.grad_matmul
        grads = gradFuntion(gradients)

        # dop = [grads[0] if self.history[1].requires_grad else np.zeros_like(self.history[1].arr),
            #    grads[1] if self.history[2].requires_grad else np.zeros_like(self.history[2].arr)]
       
        if self.history[1].requires_grad and self.history[1].history[0] == 'leaf':
            self.history[1].grad += grads[0]
        else:
            self.history[1].grad = np.zeros_like(self.history[1].arr)

        if self.history[2].requires_grad and self.history[2].history[0] == 'leaf':
            self.history[2].grad += grads[1]
        else:
            self.history[2].grad = np.zeros_like(self.history[2].arr)

        self.history[1].backward(grads[0])
        self.history[2].backward(grads[1])
