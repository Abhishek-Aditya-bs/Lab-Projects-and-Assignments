...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        #Checking whether grads need to be set to some previous value
        if gradients is None:
            g1 = g2 = np.ones(self.shape)
        else:
            g1 = g2 = gradients

        if self.history[1].requires_grad==True:
            self.history[1].grad += g1

        if self.history[2].requires_grad==True:
            self.history[2].grad += g2
    ...
    def grad_matmul(self, gradients=None):
        ...
        if gradients == None:
            grd = np.ones(self.shape)
        else:
            grd = gradients

        g1 = np.matmul(grd, np.transpose(self.history[2].arr))
        g2 = np.matmul(np.transpose(self.history[1].arr), grd)
