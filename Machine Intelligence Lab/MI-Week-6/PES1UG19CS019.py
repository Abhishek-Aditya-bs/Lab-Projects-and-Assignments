...
class SVM:
    ...
    def solve(self):
        ...
        # pipe = Pipeline([('scaler', StandardScaler()), ('svc', SVC(kernel = 'rbf', random_state = 0, C = 3))])
        pipe = Pipeline([('SCALER-1', MinMaxScaler(feature_range=(-1,1),clip=True)),('SCALER-2',Normalizer()),('SCALER-3', PowerTransformer(standardize=True)),('SVC', SVC(kernel = 'rbf',degree = 1, random_state = 0, C = 2.853810, gamma = 0.048599 ))])
