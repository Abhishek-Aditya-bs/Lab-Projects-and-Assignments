...
class AdaBoost:
    ...
    def update_weights(self, y, y_pred, sample_weights, alpha):
        ...
        # print(sample_weights[y == y_pred])
        # print(sample_weights[y != y_pred])
        
        sample_weights[y == y_pred] = sample_weights[y == y_pred] * np.exp(-alpha)
        sample_weights[y != y_pred] = sample_weights[y != y_pred] * np.exp(alpha)
    ...
    def predict(self, X):
        ...
        for i in range(len(self.alphas)):
            sum += self.alphas[i]
        pred = [(self.alphas[i] * self.stumps[i].predict(X))/sum for i in range(len(self.alphas))]
        return np.sign(np.sum((pred),axis=0).astype(int))
